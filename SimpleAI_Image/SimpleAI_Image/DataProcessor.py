...
class DataProcessor:
    ...
    def process_data(self, X, y):
        if not isinstance(X, pd.DataFrame) or 'image' not in X.columns:
            raise ValueError("Input X should be a DataFrame with a single column 'image' containing flattened images.")
        if len(X) == 0:
        ...
        try:
            X_images = np.stack(X['image'].values).reshape(-1, 28, 28, 1)
        except Exception as e:
            raise ValueError("Error reshaping images. Ensure the input images are flattened arrays of size 784.") from e
    ...
    def visualize_data(self, X_embedded, y):
        if X_embedded.size == 0 or y.size == 0:
            raise ValueError("Input data for visualization is empty.")
